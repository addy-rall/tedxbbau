  ...
  const renderWord = (word, index) => {
    if (word.toLowerCase().includes("x")) {
      return (
        ...
        >
          {word.split("x").map((part, xIndex, arr) => (
...
const App = () => {
  ...
  return (
    <>
      ...
      <div className="py-23 w-full">
        <h1
          className="text-white text-4xl md:text-5xl mt-32 font-bold absolute z-10 inset-x-0 w-full px-6"
          style={{
        ...
        >
          <div
            className="block mx-auto md:text-center leading-snug"
            style={{ textAlign: "center" }} 
          >
            <SplitText text="TEDxBBAU" />
          </div>
          <div
            className="block mx-auto md:text-center leading-snug"
            style={{ textAlign: "center" }} 
          >
            <SplitText text="Unveiling Hidden Truths" />
          </div>
        </h1>
      </div>
      ...
      <div className="w-full text-center py-45 mt-20">
        <motion.div
          className="bg-red-700 text-white text-lg md:text-xl font-bold py-4 px-8 rounded-md flicker"
          style={{ fontFamily: "Poppins, sans-serif" }}
          initial={{ opacity: 0.3 }}
          animate={{ opacity: [0.3, 1, 0.3] }}
          transition={{ repeat: Infinity, duration: 0.5 }}
        >
          Register for the event NOW! First 100 people to register get an Early Bird DISCOUNT!
        </motion.div>
      </div>
      ...
      <div className="min-h-screen w-full py-15 flex flex-col md:flex-row items-center justify-center text-white relative container mx-auto px-4">
        ...
        >
          <h1 className="text-3xl md:text-3xl font-semibold">
            Breaking Myths, Unveiling Reality
          </h1>

          <motion.p
            ref={paragraphRef}
            className="text-lg mt-4 px-2 md:px-10 text-gray-300"
            initial={{ opacity: 0, y: 50 }}
